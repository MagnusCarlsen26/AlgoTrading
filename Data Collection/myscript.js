(function() {
    ...
    function handleOrderBook() {
        ...
        if (h) {
            ...
            if (timeOfQuestion > currentTime) { sendDataToServer(yesData, noData, title,currentTime) } 
            else { location.reload() }

        } 
        else { console.error("Order book element not found.") }
    }
    ...
    setTimeout(function() {
        ...
        elements.forEach(element => {
            const investedText = element.textContent.trim()
            if (investedText.includes('Invested')) {investedFound = true }
        })

        const button = document.querySelector('.style_event__card__actions__button__107m2') 

        if (!investedFound && button) { button.click() } 
        else { console.log('Either "Invested" text was found or the button doesn\'t exist') }

        setTimeout(function() {
            socket = io('http://localhost:5000')
            setTimeout(function() {
                clickElement('.style_order__book__top__3keFx')
                setInterval(handleOrderBook, 1000)
            }, 1000)
        }, 1500)
    }, 3000)
})()
